...
def main(reprocess,userFile = "CGM11.userSettings" , processingOnly = False, expertFile="CGM1_1-pyCGM2.settings",emgFile="emg.settings",vskFile=None,plotFlag=False, overwrittenMode=True):
    modelInfo={"Version":"cgm1.1"}

    DATA_PATH = os.getcwd()+"\\"
    DATA_PATH_OUT = DATA_PATH+"Processing-CGM11\\"
    files.createDir(DATA_PATH_OUT)

    if not reprocess:
        userSettings,internalSettings,emgSettings,vsk = common.settingsManager(DATA_PATH,userFile,vskFile,expertFile,emgFile)

        builder = pipelineProcedure.CGM11PipelineBuilder(userSettings)
    ...
    if not reprocess:
        pmf.exportAQMinfos(DATA_PATH_OUT,"CGM 1.1",jsonAnalyses = jsonAnalysisInstances)
    else:
...
if __name__ == '__main__':
        ...
        parser.add_argument('--userFile', type=str, help='userSettings', default="CGM11.userSettings")
        parser.add_argument('--expertFile', type=str, help='Local expert settings',default="CGM1_1-pyCGM2.settings")
