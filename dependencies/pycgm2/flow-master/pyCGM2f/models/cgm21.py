...
def main(reprocess,userFile = "CGM21.userSettings" , processingOnly = False, expertFile="CGM2_1-pyCGM2.settings",emgFile="emg.settings",vskFile=None,plotFlag=False, overwrittenMode=True):

    modelInfo={"Version":"cgm2.1"}

    DATA_PATH = os.getcwd()+"\\"
    DATA_PATH_OUT = DATA_PATH+"Processing-CGM21\\"
    files.createDir(DATA_PATH_OUT)

    if not reprocess:
        userSettings,internalSettings,emgSettings,vsk = common.settingsManager(DATA_PATH,userFile,vskFile,expertFile,emgFile)

        builder = pipelineProcedure.CGM21PipelineBuilder(userSettings)
    ...
    if not processingOnly:
        ...
        for calibrationIt in manager.getNestedCalibrations():
            ...
            # btkTools.smartWriter(finalAcqStatic, str(DATA_PATH+calibrationIt["StaticTrial"]))
    ...
    if not reprocess:
        pmf.exportAQMinfos(DATA_PATH_OUT,"CGM 2.1",jsonAnalyses = jsonAnalysisInstances)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--userFile', type=str, help='userSettings', default="CGM21.userSettings")
    parser.add_argument('--expertFile', type=str, help='Local expert settings',default="CGM2_1-pyCGM2.settings")
