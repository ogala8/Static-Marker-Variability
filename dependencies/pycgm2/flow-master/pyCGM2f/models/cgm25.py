...
def main(reprocess,userFile = "CGM25.userSettings" , processingOnly = False, expertFile="CGM2_5-pyCGM2.settings",emgFile="emg.settings",vskFile=None,plotFlag=False, overwrittenMode=True):

    modelInfo={"Version":"cgm2.5"}

    DATA_PATH = os.getcwd()+"\\"
    DATA_PATH_OUT = DATA_PATH+"Processing\\"
    files.createDir(DATA_PATH_OUT)

    if not reprocess:
        userSettings,internalSettings,emgSettings,vsk = common.settingsManager(DATA_PATH,userFile,vskFile,expertFile,emgFile)

        builder = pipelineProcedure.CGM25PipelineBuilder(userSettings)
    ...
    if not reprocess:
        pmf.exportAQMinfos(DATA_PATH_OUT,"CGM 2.5",jsonAnalyses = jsonAnalysisInstances)
    else:
...
if __name__ == '__main__':
    arser = argparse.ArgumentParser(description='CGM2.5-pipeline')
    parser.add_argument('-r','--reprocess', action='store_true', help='reprocess data from CGA.info' )
    parser.add_argument('--userFile', type=str, help='userSettings', default="CGM25.userSettings")
    parser.add_argument('--expertFile', type=str, help='Local expert settings',default="CGM2_5-pyCGM2.settings")
