...
function Trial = DefineSegments_Omar_CGM24(Participant,Static,Trial)
...
RTHI = permute(Trial.Marker(10).Trajectory.smooth,[2,3,1]);
RKNE = permute(Trial.Marker(11).Trajectory.smooth,[2,3,1]);
RKNM = permute(Trial.Marker(12).Trajectory.smooth,[2,3,1]);
RTHAP = permute(Trial.Marker(69).Trajectory.smooth,[2,3,1]);
RTHAD = permute(Trial.Marker(70).Trajectory.smooth,[2,3,1]);
% Knee joint centre
RKJC = (RKNE+RKNM)/2;
% Store virtual marker
